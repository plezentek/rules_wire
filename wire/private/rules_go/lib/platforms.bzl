...
def _generate_constraints(names, bazel_constraints):
    return {
        name: bazel_constraints.get(name, "@com_plezentek_rules_wire//wire/toolchain:" + name)
        for name in names
    }
...
def _generate_platforms():
    ...
    for goos, goarch in GOOS_GOARCH:
        ...
        platforms.append(struct(
            ...
            goos = goos,
            goarch = goarch,
            constraints = constraints + ["@com_plezentek_rules_wire//wire/toolchain:cgo_off"],
            cgo = False,
        ))
        if (goos, goarch) in CGO_GOOS_GOARCH:
            ...
            platforms.append(struct(
                ...
                goos = goos,
                goarch = goarch,
                constraints = constraints + ["@com_plezentek_rules_wire//wire/toolchain:cgo_on"] + mingw,
                cgo = True,
            ))
    ...
    for goarch in ("arm", "arm64", "386", "amd64"):
        ...
        platforms.append(struct(
            ...
            goos = "darwin",
            goarch = goarch,
            constraints = constraints + ["@com_plezentek_rules_wire//wire/toolchain:cgo_on"],
            cgo = True,
        ))
...
def generate_toolchain_names():
    ...
    return ["wire_" + p.name for p in PLATFORMS if not p.cgo]
